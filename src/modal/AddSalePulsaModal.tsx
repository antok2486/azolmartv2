...
const AddSalePulsaComp = ({ ...props }) => {
    ...
    return (
        <View style={{ height: '100%', width: '100%', backgroundColor: '#fff', }}>
            ...
            <View style={{ padding: 10 }}>
                <Text style={{ fontWeight: 'bold', marginBottom: 10 }}>Harga Jual</Text>
                <TextInput style={style.textInput} defaultValue={props.namaCustomer} onChangeText={(text) => handleChange('hrg_jual', text)} />
            </View>
        </View>
    )
}

export default function AddSalePulsaModal({navigation, route}) {
    ...
    const save = async () => {
        ...
        let payload = { 'trsalp': data }
        console.log(payload)
        try {
            let res = await axios.put(URL_API + 'trsalp', payload, config)
    ...
    return (
        <Stack.Navigator>
            <Stack.Screen
                name='addsaleComp'
                options={{
                    headerTitle: 'Input Penjualan Pulsa/Token',
                    headerStyle: {
            ...
            >
                {(props) => <AddSalePulsaComp {...props} data={data} setData={setData} />}
            </Stack.Screen>
        </Stack.Navigator>
